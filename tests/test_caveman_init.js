...
// Isolate the OpenClaw target: point OPENCLAW_WORKSPACE at a non-existent path
// under the test's tmp dir so init NEVER writes to the real ~/.openclaw/workspace
// and the openclaw target is skipped deterministically ("workspace missing",
// no --force). Without this, the suite mutates the user's actual OpenClaw config.
function initEnv(tmp) {
  return { ...process.env, OPENCLAW_WORKSPACE: path.join(tmp, 'no-openclaw-ws') };
}
...
test('greenfield: creates all rule files with proper frontmatter', (tmp) => {
  execFileSync(process.execPath, [INIT, tmp], { encoding: 'utf8', env: initEnv(tmp) });
...
test('idempotent: re-running on a clean install skips all', (tmp) => {
  execFileSync(process.execPath, [INIT, tmp], { encoding: 'utf8', env: initEnv(tmp) });
  const out = execFileSync(process.execPath, [INIT, tmp], { encoding: 'utf8', env: initEnv(tmp) });
...
test('--force overwrites existing rule files', (tmp) => {
  ...
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'caveman.mdc'), '# original\n');
  execFileSync(process.execPath, [INIT, tmp, '--force'], { encoding: 'utf8', env: initEnv(tmp) });
  const after = fs.readFileSync(path.join(dir, 'caveman.mdc'), 'utf8');
...
test('--dry-run: announces but writes nothing', (tmp) => {
  const out = execFileSync(process.execPath, [INIT, tmp, '--dry-run'], { encoding: 'utf8', env: initEnv(tmp) });
...
test('--only filters to one target', (tmp) => {
  const out = execFileSync(process.execPath, [INIT, tmp, '--only', 'cline'], { encoding: 'utf8', env: initEnv(tmp) });
