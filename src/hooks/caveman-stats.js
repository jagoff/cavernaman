...
function main() {
  ...
  const mode = readFlag(path.join(claudeDir, '.cavernaman-active'));
  ...
  if (parsed.turns > 0) {
    ...
    // user-owned, same symlink-clobber surface as the .cavernaman-active flag.
    const agg = aggregateHistory(historyPath, null);
    const suffix = agg.estSavedTokens > 0 ? `⛏ ${humanizeTokens(agg.estSavedTokens)}` : '';
    safeWriteFlag(path.join(claudeDir, '.cavernaman-statusline-suffix'), suffix);
  }
