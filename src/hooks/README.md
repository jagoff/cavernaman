...
- Shows `[CAVERNAMAN]`, `[CAVERNAMAN:ULTRA]`, `[CAVERNAMAN:WENYAN]`, etc.
...
if [ -f "$caveman_flag" ]; then
  ...
  if [ "$caveman_mode" = "full" ] || [ -z "$caveman_mode" ]; then
    caveman_text=$'\033[38;5;172m[CAVERNAMAN]\033[0m'
  else
    caveman_suffix=$(echo "$caveman_mode" | tr '[:lower:]' '[:upper:]')
    caveman_text=$'\033[38;5;172m[CAVERNAMAN:'"${caveman_suffix}"$']\033[0m'
  fi
fi
```
...
Badge examples:
- `/caveman` → `[CAVERNAMAN]`
- `/caveman ultra` → `[CAVERNAMAN:ULTRA]`
- `/caveman wenyan` → `[CAVERNAMAN:WENYAN]`
- `/caveman-commit` → `[CAVERNAMAN:COMMIT]`
- `/caveman-review` → `[CAVERNAMAN:REVIEW]`
