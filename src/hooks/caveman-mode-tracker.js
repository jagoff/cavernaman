...
process.stdin.on('end', () => {
  try {
    ...
    if (/\b(activate|enable|turn on|start|talk like)\b.*\b(?:cavernaman|caveman)\b/i.test(prompt) ||
        /\b(?:cavernaman|caveman)\b.*\b(mode|activate|enable|turn on|start)\b/i.test(prompt)) {
    ...
    // Match /cavernaman (main) and /caveman-* (sub-skills) commands
    if (prompt.startsWith('/cavernaman') || prompt.startsWith('/caveman')) {
      const parts = prompt.split(/\s+/);
      const cmd = parts[0]; // /cavernaman, /caveman-commit, /caveman-review, etc.
      const arg = parts[1] || '';
      ...
      if (cmd === '/caveman-commit') {
        ...
        mode = 'compress';
      } else if (cmd === '/cavernaman' || cmd === '/cavernaman:cavernaman' || cmd === '/caveman' || cmd === '/caveman:caveman') {
        // Bare /cavernaman (or legacy /caveman) → activate at configured default
        if (!arg) {
          mode = getDefaultMode();
    ...
    if (/\b(stop|disable|deactivate|turn off)\b.*\b(?:cavernaman|caveman)\b/i.test(prompt) ||
        /\b(?:cavernaman|caveman)\b.*\b(stop|disable|deactivate|turn off)\b/i.test(prompt) ||
    ...
    if (activeMode && !INDEPENDENT_MODES.has(activeMode)) {
      process.stdout.write(JSON.stringify({
        hookSpecificOutput: {
          ...
          additionalContext: "Cavernaman ON (" + activeMode + "). " +
