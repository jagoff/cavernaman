...
FLAG="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.cavernaman-active"
...
if [ -z "$MODE" ] || [ "$MODE" = "full" ]; then
  printf '\033[38;5;172m[CAVERNAMAN]\033[0m'
else
  SUFFIX=$(printf '%s' "$MODE" | tr '[:lower:]' '[:upper:]')
  printf '\033[38;5;172m[CAVERNAMAN:%s]\033[0m' "$SUFFIX"
fi
...
if [ "${CAVEMAN_STATUSLINE_SAVINGS:-1}" != "0" ]; then
  SAVINGS_FILE="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.cavernaman-statusline-suffix"
